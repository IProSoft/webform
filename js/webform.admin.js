...
(function ($, Drupal, debounce, once) {
    ...
    attach(context) {
      $(
        once(
          ...
          '.webform-filter-form input.form-autocomplete',
          context,
        ),
      ).each(function () {
  ...
  Drupal.behaviors.webformTableRowHref = {
    attach(context) {
      ...
      $(once('webform-results-table', '.webform-results-table', context)).on(
        'click',
        function (event) {
          if (
          ...
          if (
            ...
            $(event.target).parents('.dropbutton-widget').length
          ) {
            return true;
          }

          const $input = $(event.target).closest('td').find('input');
          if ($input.length) {
          ...
          const $tr = $(event.target).parents('tr[data-webform-href]');
          if (!$tr.length) {
            return true;
          }
