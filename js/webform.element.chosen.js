...
(function ($, Drupal, once) {
  ...
  Drupal.webform.chosen.options.width =
    Drupal.webform.chosen.options.width || '100%';
  Drupal.webform.chosen.options.widthInline =
    Drupal.webform.chosen.options.widtInline || '50%';
  ...
  Drupal.behaviors.webformChosen = {
    attach(context) {
      ...
      $(
        once(
          'webform-chosen',
          'select.js-webform-chosen, .js-webform-chosen select',
          context,
        ),
      ).each(function () {
        const $select = $(this);
  ...
  $document.on('state:disabled', function (e) {
    const $chosen = $(e.target).find('.js-webform-chosen');
