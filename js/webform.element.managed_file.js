...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.webformManagedFileAutoUpload = {
    attach: function attach(context) {
      ...
      $(once('webform-auto-file-upload', 'form', context)).on(
        'submit',
        function (event) {
          const $form = $(this);
          if (
            $form.data('webform-auto-file-uploads') > 0 &&
      ...
      if (
        ...
        typeof Drupal.Ajax.prototype
          .beforeSubmitWebformManagedFileAutoUploadOriginal === 'undefined'
      ) {
        ...
        Drupal.Ajax.prototype.beforeSubmit = function (
          ...
          const isFormActions = $element.closest('.form-actions').length;

          // Determine if the triggering element is within a multiple element.
          const isMultipleUpload = $element
            .parents(
              '.js-form-type-webform-multiple, .js-form-type-webform-custom-composite',
            )
            .find('.js-form-managed-file').length;

          // Determine if the triggering element is not within a
          // managed file element.
          const isManagedUploadButton = $element.parents(
          ...
          return this.beforeSubmitWebformManagedFileAutoUploadOriginal.apply(
            this,
            arguments,
          );
        };
      ...
      $(once('webform-auto-file-upload', 'input[type="file"]', context)).on(
        'change',
        function () {
          // Track file upload.
          ...
          const $form = $(this.form);
          const fileUploads = $form.data('webform-auto-file-uploads') || 0;
    ...
    detach: function detach(context, settings, trigger) {
      if (trigger === 'unload') {
        const removedElements = once.remove(
          'webform-auto-file-upload',
          'input[type="file"]',
          context,
        );
