...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.webformUnsaved = {
    ...
    attach(context) {
      ...
      if (
        $(
          once(
            'data-webform-unsaved',
            '.js-webform-unsaved[data-webform-unsaved]',
          ),
        ).length
      ) {
        unsaved = true;
        ...
        $(
          once(
            'webform-unsaved',
            $(
              '.js-webform-unsaved :input:not(:button, :submit, :reset, [type="hidden"])',
            ),
          ),
        ).on('change keypress', function (event, param1) {
      ...
      $(
        once(
          'webform-unsaved',
          $(
            '.js-webform-unsaved button, .js-webform-unsaved input[type="submit"]',
            context,
          ),
        ),
      )
      ...
      if (
        ...
        typeof Drupal.Ajax.prototype.beforeSubmitWebformUnsavedOriginal ===
          'undefined'
      ) {
        Drupal.Ajax.prototype.beforeSubmitWebformUnsavedOriginal =
  ...
  $(function () {
    ...
    const isIOSorOpera =
      navigator.userAgent.toLowerCase().match(/iphone|ipad|ipod|opera/) ||
    ...
    $('a:not(.use-ajax)').bind('click', function (evt) {
      ...
      if (
        ...
        !(href.match(/^#/) || href.trim() === '')
      ) {
        if ($(window).triggerHandler('beforeunload')) {
          if (
            !window.confirm(
              `${Drupal.t('Changes you made may not be saved.')}\n\n${Drupal.t(
                'Press OK to leave this page or Cancel to stay.',
              )}`,
            )
          ) {
