...
(function ($, Drupal, once) {
  ...
  $.fn.isWebformElement = function () {
    return !!(
      $(this).isWebform() || $(this).closest('[data-is-webform-element]').length
    );
  };
  ...
  Drupal.states.Dependent.comparisons.Object = function (reference, value) {
    ...
    if ('greater_equal' in reference) {
      return (
        value !== '' && parseFloat(reference.greater_equal) <= parseFloat(value)
      );
    }
  ...
  $document.on('state:visible-slide', function (e) {
    if (e.trigger && $(e.target).isWebformElement()) {
      ...
      const duration = Drupal.webform.states[effect].duration;
      $(e.target)
  ...
  $document.on('state:disabled', function (e) {
    if (e.trigger && $(e.target).isWebformElement()) {
      ...
      const fileElements = $(e.target).find(
