...
class WebformAccessHooks
{
     ...
     */
    #[Hook('webform_access')]
    public function webformAccess(\Drupal\webform\WebformInterface $webform, $operation, \Drupal\Core\Session\AccountInterface $account)
    {
        ...
        foreach ($webform_access_groups as $webform_access_group) {
            ...
            if (in_array('administer', $permissions) || in_array($permission, $permissions)) {
                return \Drupal\Core\Access\AccessResult::allowed()->cachePerUser()->addCacheableDependency($webform_access_group);
            }
        }
        // No opinion.
        return \Drupal\Core\Access\AccessResult::neutral();
    ...
    public function formUserFormAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state)
    {
        // Make sure some webform access groups exist before displaying
        // the webform access details widget.
        if (!\Drupal\webform_access\Entity\WebformAccessGroup::loadMultiple()) {
            return;
        }
