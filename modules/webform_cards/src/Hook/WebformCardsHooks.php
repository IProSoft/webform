...
class WebformCardsHooks
{
    ...
    public function webformSubmissionFormAlter(array &$form, \Drupal\Core\Form\FormStateInterface $form_state, $form_id)
    {
        ...
        if ($form_object->getOperation() === 'test' && \Drupal::request()->getMethod() === 'GET') {
            ...
            $build = ['#type' => 'link', '#url' => \Drupal\Core\Url::fromRoute('<none>', [], ['fragment' => $form_id]), '#title' => t('Submit %title form', ['%title' => $webform->label()]), '#attributes' => ['class' => ['js-webform-card-test-submit-form']]];
        ...
        if ($current_page !== \Drupal\webform\WebformInterface::PAGE_CONFIRMATION) {
            ...
            if (!in_array($current_page, [\Drupal\webform\WebformInterface::PAGE_PREVIEW, \Drupal\webform\WebformInterface::PAGE_CONFIRMATION])) {
                $current_page = $current_card ?: key($pages);
            }
            $form['progress'] = ['#theme' => 'webform_progress', '#webform' => $webform, '#webform_submission' => $webform_submission, '#pages' => $pages, '#current_page' => $current_page, '#operation' => $form_object->getOperation(), '#weight' => -20];
        }
        ...
        $form['actions']['cards_prev'] = [
            ...
            '#attributes' => ['class' => ['webform-button--previous', 'webform-cards-button--previous']],
            '#weight' => 0,
        ...
        $form['actions']['cards_next'] = ['#type' => 'submit', '#value' => $webform->getSetting('wizard_next_button_label', TRUE), '#attributes' => ['class' => ['webform-button--next', 'webform-cards-button--next']], '#weight' => 1];
