...
class WebformCardsHooks {
  ...
  public function webformSubmissionFormAlter(array &$form, FormStateInterface $form_state, $form_id) {
    ...
    if ($form_object->getOperation() === 'test' && \Drupal::request()->getMethod() === 'GET') {
      ...
      $build = ['#type' => 'link', '#url' => Url::fromRoute('<none>', [], ['fragment' => $form_id]), '#title' => t('Submit %title form', ['%title' => $webform->label()]), '#attributes' => ['class' => ['js-webform-card-test-submit-form']]];
    ...
    if ($current_page !== WebformInterface::PAGE_CONFIRMATION) {
      ...
      if (!in_array($current_page, [WebformInterface::PAGE_PREVIEW, WebformInterface::PAGE_CONFIRMATION])) {
