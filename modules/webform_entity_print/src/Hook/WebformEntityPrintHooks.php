    ...
    public function fileDownload($uri)
    {
        ...
        if ($signature_elements && preg_match_all('#/webform/' . $webform_id . '/(' . implode('|', $signature_elements) . ')/#', $uri, $matches)) {
            ...
            if ($webform_entity_print_token === $encrypt_token) {
                ...
                return ['Content-Type' => 'image/png', 'Content-Length' => $filesize, 'Cache-Control' => 'private', 'Content-Disposition' => \Symfony\Component\HttpFoundation\HeaderUtils::makeDisposition(\Symfony\Component\HttpFoundation\HeaderUtils::DISPOSITION_INLINE, (string) $filename)];
            }
        }
        return NULL;
    }
