...
class WebformExamplesAccessibilityHooks {
  ...
  public function webformSubmissionFormAlter(array &$form, FormStateInterface $form_state, $form_id) {
    ...
    $form['accessibility']['accessibility'] = ['#type' => 'link', '#title' => $accessibility ? t('Hide accessibility') : t('Show accessibility'), '#url' => Url::fromRoute('<current>', [], ['query' => ['accessibility' => $accessibility ? 0 : 1] + $query])];
    /* ************************************************************************ */
    // Required.
    /* ************************************************************************ */
    ...
    $form['accessibility']['required'] = ['#type' => 'link', '#title' => $required ? t('Disable required') : t('Enable required'), '#url' => Url::fromRoute('<current>', [], ['query' => ['required' => $required ? 0 : 1] + $query])];
    if ($required !== NULL) {
    ...
    $form['accessibility']['novalidate'] = ['#type' => 'link', '#title' => $novalidate ? t('Enable client-side validation') : t('Disable client-side validation'), '#url' => Url::fromRoute('<current>', [], ['query' => ['novalidate' => $novalidate ? 0 : 1] + $query])];
    /* ************************************************************************ */
    // Inline form error.
    /* ************************************************************************ */
    if (\Drupal::moduleHandler()->moduleExists('inline_form_errors')) {
      ...
      $form['accessibility']['disable_inline_form_errors'] = ['#type' => 'link', '#title' => $disable_inline_form_errors ? t('Enable inline form errors') : t('Disable inline form errors'), '#url' => Url::fromRoute('<current>', [], ['query' => ['disable_inline_form_errors' => $disable_inline_form_errors ? 0 : 1] + $query])];
    }
  }
