...
class WebformIcheckHooks
{
    ...
    {
        $libraries = [];
        $libraries['jquery.icheck'] = ['title' => t('jQuery: iCheck'), 'description' => t('Highly customizable checkboxes and radio buttons.'), 'notes' => t('iCheck is used to optionally enhance checkboxes and radio buttons.'), 'homepage_url' => \Drupal\Core\Url::fromUri('http://icheck.fronteed.com/'), 'download_url' => \Drupal\Core\Url::fromUri('https://github.com/dargullin/icheck/archive/refs/tags/1.0.2.zip'), 'version' => '1.0.2 ', 'optional' => FALSE, 'deprecated' => t('The iCheck library is not being maintained. It has been <a href=":href">deprecated</a> and will be removed in Webform 7.0.', [':href' => 'https://www.drupal.org/project/webform/issues/2931154']), 'license' => 'MIT'];
        return $libraries;
    }
    /**
    ...
    public function webformElementConfigurationFormAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state)
    {
        ...
        $form['form']['icheck'] = ['#type' => 'select', '#title' => t('Enhance using iCheck'), '#description' => t('Replaces @type element with jQuery <a href=":href">iCheck</a> boxes.', ['@type' => mb_strtolower($element_plugin->getPluginLabel()), ':href' => 'http://icheck.fronteed.com/']), '#empty_option' => t('- Default -'), '#options' => _webform_icheck_get_options()];
        if ($default_icheck) {
            $icheck_options = \Drupal\Core\Form\OptGroup::flattenOptions($form['form']['icheck']['#options']);
    ...
    public function webformAdminThirdPartySettingsFormAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state)
    {
        ...
        $form['third_party_settings']['webform_icheck']['default_icheck'] = ['#type' => 'select', '#title' => t('Enhance checkboxes/radio buttons using iCheck'), '#description' => t('If set, all checkboxes/radio buttons with be enhanced using jQuery <a href=":href">iCheck</a> boxes.', [':href' => 'http://icheck.fronteed.com/']), '#empty_option' => t('- Default -'), '#options' => _webform_icheck_get_options(), '#default_value' => $default_icheck];
    }
}
