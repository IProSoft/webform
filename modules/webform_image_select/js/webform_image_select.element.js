...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.webformImageSelect = {
    attach(context) {
      ...
      $(once('webform-image-select', '.js-webform-image-select', context)).each(
        function () {
          const $select = $(this);
          const isMultiple = $select.attr('multiple');

          // Apply image data to options.
          const images = JSON.parse($select.attr('data-images'));
          ...
          const $thumbnail = $select
            .next('.image_picker_selector')
            .find('.thumbnail');
          $thumbnail
