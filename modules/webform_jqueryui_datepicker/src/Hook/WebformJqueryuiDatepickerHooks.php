...
class WebformJqueryuiDatepickerHooks {
  ...
  public function webformElementConfigurationFormAlter(&$form, FormStateInterface $form_state) {
    ...
    // Date element.
    if ($element_plugin instanceof Date) {
      $form['date']['datepicker'] = ['#type' => 'checkbox', '#title' => t('Use date picker'), '#description' => t('If checked, the HTML5 date element will be replaced with a <a href="https://jqueryui.com/datepicker/">jQuery UI datepicker</a>'), '#return_value' => TRUE];
      $form['date']['datepicker_button'] = ['#type' => 'checkbox', '#title' => t('Show date picker button'), '#description' => t('If checked, date picker will include a calendar button'), '#return_value' => TRUE, '#states' => ['visible' => [':input[name="properties[datepicker]"]' => ['checked' => TRUE]]]];
      $date_format = DateFormat::load('html_date')->getPattern();
      $form['date']['date_date_format'] = ['#type' => 'webform_select_other', '#title' => t('Date format'), '#options' => [$date_format => t('HTML date - @format (@date)', ['@format' => $date_format, '@date' => _webform_jqueryui_datepicker_format_date($date_format)]), 'l, F j, Y' => t('Long date - @format (@date)', ['@format' => 'l, F j, Y', '@date' => _webform_jqueryui_datepicker_format_date('l, F j, Y')]), 'D, m/d/Y' => t('Medium date - @format (@date)', ['@format' => 'D, m/d/Y', '@date' => _webform_jqueryui_datepicker_format_date('D, m/d/Y')]), 'm/d/Y' => t('Short date - @format (@date)', ['@format' => 'm/d/Y', '@date' => _webform_jqueryui_datepicker_format_date('m/d/Y')])], '#description' => t("Date format is only applicable for browsers that do not have support for the HTML5 date element. Browsers that support the HTML5 date element will display the date using the user's preferred format."), '#other__option_label' => t('Custom…'), '#other__placeholder' => t('Custom date format…'), '#other__description' => t('Enter date format using <a href="http://php.net/manual/en/function.date.php">Date Input Format</a>.'), '#attributes' => ['data-webform-states-no-clear' => TRUE], '#states' => ['visible' => [':input[name="properties[datepicker]"]' => ['checked' => TRUE]]]];
      ...
      $form['date']['date_container']['step'] = ['#type' => 'number', '#title' => t('Step'), '#description' => t('Specifies the legal number intervals.'), '#min' => 1, '#size' => 4, '#states' => ['invisible' => [':input[name="properties[datepicker]"]' => ['checked' => TRUE]]]];
    }
    // Datetime element.
    if ($element_plugin instanceof DateTime) {
      ...
      // Add button support.
      $form['date']['date_date_datepicker_button'] = ['#type' => 'checkbox', '#title' => t('Show date picker button'), '#description' => t('If checked, date picker will include a calendar button'), '#return_value' => TRUE, '#states' => ['visible' => [':input[name="properties[date_date_element]"]' => ['value' => 'datepicker']]]];
      // Adjust weights.
      $form['date']['date_date_element']['#weight'] = -10;
      $form['date']['date_date_datepicker_button']['#weight'] = -9;
      // Adjust states.
      $form['date']['date_date_placeholder']['#states']['visible'] = [[':input[name="properties[date_date_element]"]' => ['value' => 'text']], 'or', [':input[name="properties[date_date_element]"]' => ['value' => 'datepicker']]];
      $form['date']['date_date_format']['#states']['visible'] = [[':input[name="properties[date_date_element]"]' => ['value' => 'text']], 'or', [':input[name="properties[date_date_element]"]' => ['value' => 'datepicker']]];
    }
  }
