    ...
    attach(context) {
      ...
      $(
        once(
          ...
          '.js-webform-type-webform-location-geocomplete',
          context,
        ),
      ).each(function () {
        const $element = $(this);
        const $input = $element.find('.webform-location-geocomplete');

        // Display a map.
        let $map = null;
        if ($input.attr('data-webform-location-geocomplete-map')) {
          $map = $(
            '<div class="webform-location-geocomplete-map"><div class="webform-location-geocomplete-map--container"></div></div>',
          )
            .insertAfter($input)
            .find('.webform-location-geocomplete-map--container');
        }
        ...
        const $geocomplete = $input.geocomplete(options);
        ...
        const value = $geocomplete.val();
        if (value) {
          $geocomplete.geocomplete('find', value);
        } else if (
          window.navigator.geolocation &&
          $geocomplete.attr('data-webform-location-geocomplete-geolocation')
        ) {
