...
class WebformLocationGeocompleteHooks
{
    ...
    {
        $libraries = [];
        $libraries['jquery.geocomplete'] = ['title' => t('jQuery: Geocoding and Places Autocomplete Plugin'), 'description' => t("Geocomple is an advanced jQuery plugin that wraps the Google Maps API's Geocoding and Places Autocomplete services."), 'notes' => t('Geocomplete is used by the location element.'), 'homepage_url' => \Drupal\Core\Url::fromUri('http://ubilabs.github.io/geocomplete/'), 'download_url' => \Drupal\Core\Url::fromUri('https://github.com/ubilabs/geocomplete/archive/refs/tags/1.7.0.zip'), 'version' => '1.7.0', 'elements' => ['webform_location_geocomplete'], 'optional' => FALSE, 'deprecated' => t('The jQuery: Geocoding and Places Autocomplete Plugin library is not being maintained. It has been <a href=":href">deprecated</a> and will be removed in Webform 7.0.', [':href' => 'https://www.drupal.org/node/2991275']), 'license' => 'MIT'];
        return $libraries;
    }
    /**
     ...
     */
    #[Hook('js_alter')]
    public function jsAlter(&$javascript, \Drupal\Core\Asset\AttachedAssetsInterface $assets)
    {
    ...
    public function webformAdminThirdPartySettingsFormAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state)
    {
        ...
        $form['third_party_settings']['webform_location_geocomplete'] = ['#type' => 'details', '#title' => t('Location geocomplete'), '#open' => TRUE];
        $form['third_party_settings']['webform_location_geocomplete']['default_google_maps_api_key'] = ['#type' => 'textfield', '#title' => t('Google Maps API key'), '#description' => t('Google requires users to use a valid API key. Using the <a href="https://console.developers.google.com/apis">Google API Manager</a>, you can enable the <em>Google Maps JavaScript API</em>. That will create (or reuse) a <em>Browser key</em> which you can paste here.'), '#default_value' => $default_google_maps_api_key];
    }
}
