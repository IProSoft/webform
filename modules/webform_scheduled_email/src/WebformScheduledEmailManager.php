...
class WebformScheduledEmailManager implements WebformScheduledEmailManagerInterface {
  ...
  public function schedule(EntityInterface $entity, $handler_id) {
    if ($entity instanceof WebformSubmissionInterface) {
      ...
      if ($send_iso_date === FALSE) {
        ...
        return static::EMAIL_UNSCHEDULED;
      }
      ...
      if (!$scheduled_email) {
        ...
        if ($scheduled_email->send != $send_timestamp) {
          ...
          $status = static::EMAIL_RESCHEDULED;
        }
        else {
          $action = NULL;
          $status = static::EMAIL_ALREADY_SCHEDULED;
        }
      }
      ...
      $query->fields([
        ...
        'state' => static::SUBMISSION_SEND,
        'send' => $send_timestamp,
      ])->execute();

      // If email is alread scheduled when don't need to log anything.
      if ($status === static::EMAIL_ALREADY_SCHEDULED) {
        return $status;
      }
    ...
    elseif ($entity instanceof WebformInterface) {
      ...
      $this->database->update('webform_scheduled_email')
        ->fields(['state' => static::SUBMISSION_SCHEDULE])
      ...
      $query->addExpression("'" . static::SUBMISSION_SCHEDULE . "'", 'state');
      $query->condition('webform_id', $webform->id());
      if ($sids) {
  ...
  public function unschedule(EntityInterface $entity, $handler_id = NULL) {
    ...
    elseif ($entity instanceof WebformInterface) {
      $webform = $entity;
      $query = $this->database->update('webform_scheduled_email')
        ->fields(['state' => static::SUBMISSION_UNSCHEDULE])
        ->condition('webform_id', $webform->id());
      if ($handler_id) {
        $query->condition('handler_id', $handler_id);
      }
  ...
  public function reschedule(EntityInterface $entity, $handler_id = NULL) {
    if ($entity instanceof WebformSubmissionInterface) {
      $webform_submission = $entity;
      $query = $this->database->update('webform_scheduled_email')
        ->fields(['state' => static::SUBMISSION_RESCHEDULE])
  ...
  public function cron(EntityInterface $entity = NULL, $handler_id = NULL, $schedule_limit = 1000, $send_limit = NULL) {
    ...
    // Build summary.
    $labels = [
      static::EMAIL_SCHEDULED => $this->t('scheduled'),
      static::EMAIL_RESCHEDULED => $this->t('rescheduled'),
      static::EMAIL_ALREADY_SCHEDULED => $this->t('already scheduled'),
      static::EMAIL_UNSCHEDULED => $this->t('unscheduled'),
      static::EMAIL_SENT => $this->t('sent'),
      static::EMAIL_NOT_SENT => $this->t('not sent'),
      static::EMAIL_SKIPPED => $this->t('skipped'),
    ];
    $summary = [];
  ...
  protected function cronSchedule(EntityInterface $entity = NULL, $handler_id = NULL, $limit = 1000) {
    $stats = [
      static::EMAIL_SCHEDULED => 0,
      static::EMAIL_RESCHEDULED => 0,
      static::EMAIL_UNSCHEDULED => 0,
      static::EMAIL_ALREADY_SCHEDULED => 0,
    ];
    ...
    $query = $this->database->select('webform_scheduled_email', 'w')
      ...
      ->condition('w.state', [static::SUBMISSION_SCHEDULE, static::SUBMISSION_UNSCHEDULE, static::SUBMISSION_RESCHEDULE], 'IN')
      ->orderBy('w.send')
      ->orderBy('w.sid')
      ->range(0, $limit);
    $this->addQueryConditions($query, $webform, $webform_submission, $source_entity, $handler_id);
    ...
    foreach ($records as $record) {
      ...
      switch ($record->state) {
        case static::SUBMISSION_SCHEDULE:
        case static::SUBMISSION_RESCHEDULE:
          $email_status = $this->schedule($webform_submission, $handler_id);
          $stats[$email_status]++;
          break;

        case static::SUBMISSION_UNSCHEDULE:
          $this->unschedule($webform_submission, $handler_id);
          $stats[static::EMAIL_UNSCHEDULED]++;
          break;
      }
    }
  ...
  protected function cronSend(EntityInterface $entity = NULL, $handler_id = NULL, $limit = 500) {
    $stats = [
      static::EMAIL_SENT => 0,
      static::EMAIL_NOT_SENT => 0,
      static::EMAIL_SKIPPED => 0,
    ];
    if (empty($limit)) {
      return $stats;
    }
    ...
    $query = $this->database->select('webform_scheduled_email', 'w')
      ...
      ->condition('w.state', static::SUBMISSION_SEND)
    ...
    foreach ($result as $record) {
      ...
      if (!$handler->checkConditions($webform_submission)) {
        ...
        $stat = ($status) ? static::EMAIL_SENT : static::EMAIL_NOT_SENT;
      }
  ...
  public function stats(EntityInterface $entity = NULL, $handler_id = NULL) {
    return [
      static::SUBMISSION_WAITING => $this->waiting($entity, $handler_id),
      static::SUBMISSION_QUEUED => $this->queued($entity, $handler_id),
      static::SUBMISSION_READY => $this->ready($entity, $handler_id),
      static::SUBMISSION_TOTAL => $this->total($entity, $handler_id),
    ];
  }
  ...
  public function waiting(EntityInterface $entity = NULL, $handler_id = NULL) {
    return $this->total($entity, $handler_id, static::SUBMISSION_WAITING);
  }
  ...
  public function queued(EntityInterface $entity = NULL, $handler_id = NULL) {
    return $this->total($entity, $handler_id, static::SUBMISSION_QUEUED);
  }
  ...
  public function ready(EntityInterface $entity = NULL, $handler_id = NULL) {
    return $this->total($entity, $handler_id, static::SUBMISSION_READY);
  }
  ...
  protected function addQueryConditions($query, WebformInterface $webform = NULL, WebformSubmissionInterface $webform_submission = NULL, EntityInterface $source_entity = NULL, $handler_id = NULL, $state = NULL) {
    ...
    switch ($state) {
      ...
      case static::SUBMISSION_UNSCHEDULE:
      case static::SUBMISSION_RESCHEDULE:
      case static::SUBMISSION_SEND:
        $query->condition($prefix . 'state', $state);
        break;

      case static::SUBMISSION_WAITING:
        $query->condition($prefix . 'state', static::SUBMISSION_SEND, '<>');
        break;

      case static::SUBMISSION_QUEUED:
      case static::SUBMISSION_READY:
        $query->condition($prefix . 'state', static::SUBMISSION_SEND);
        $query->isNotNull($prefix . 'send');
        $query->condition($prefix . 'send', time(), ($state === static::SUBMISSION_QUEUED) ? '>=' : '<');
        $query->condition($prefix . 'state', static::SUBMISSION_SEND);
        break;
    }
  }
