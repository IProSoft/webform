...
class WebformCliService implements WebformCliServiceInterface {
  ...
  public function drush_webform_libraries_composer() {
    ...
    $composer_path = \Drupal::service('extension.list.module')->getPath('webform') . '/composer.json';
  ...
  public function drush_webform_docs() {
    ...
    $html_directory_path = \Drupal::service('extension.list.module')->getPath('webform') . '/html';
  ...
  public function drush_webform_generate_commands() {
    ...
    $filepath = DRUPAL_ROOT . '/' . \Drupal::service('extension.list.module')->getPath('webform') . '/drush/webform.drush.inc';
    file_put_contents($filepath, $commands);
    $this->drush_print("$filepath updated.");

    // Drush 9.x.
    $commands = $this->drush_webform_generate_commands_drush9();
    $filepath = DRUPAL_ROOT . '/' . \Drupal::service('extension.list.module')->getPath('webform') . '/src/Commands/WebformCommands.php';
    file_put_contents($filepath, $commands);
    $this->drush_print("$filepath updated.");
  }
