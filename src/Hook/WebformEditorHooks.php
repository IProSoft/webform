...
class WebformEditorHooks
{
    ...
    public function filterFormatAccess(\Drupal\filter\FilterFormatInterface $filter_format, $operation, \Drupal\Core\Session\AccountInterface $account)
    {
        if ($filter_format->id() === \Drupal\webform\Element\WebformHtmlEditor::DEFAULT_FILTER_FORMAT && $operation !== 'use') {
            return \Drupal\Core\Access\AccessResult::forbidden('This text format can only be managed by the Webform module.');
        }
    ...
    public function filterFormatLoad($entities)
    {
        foreach ($entities as $entity) {
            ...
            if ($entity->id() === \Drupal\webform\Element\WebformHtmlEditor::DEFAULT_FILTER_FORMAT && \Drupal::routeMatch()->getRouteName() === 'filter.admin_overview') {
    ...
    {
        $settings['editor']['formats'][\Drupal\webform\Element\WebformHtmlEditor::DEFAULT_FILTER_FORMAT]['editorSettings']['autoGrow_minHeight'] = '80';
    }
    /* ************************************************************************** */
     ...
     */
    #[Hook('webform_insert')]
    public function webformInsert(\Drupal\webform\WebformInterface $webform)
    {
     ...
     */
    #[Hook('webform_update')]
    public function webformUpdate(\Drupal\webform\WebformInterface $webform)
    {
