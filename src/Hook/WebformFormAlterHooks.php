    ...
    public function formAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state, $form_id)
    {
        switch ($form_id) {
            ...
            case 'field_config_edit_form':
                ...
                \Drupal\Component\Utility\NestedArray::unsetValue($form, ['settings', 'allowed_formats', '#options', \Drupal\webform\Element\WebformHtmlEditor::DEFAULT_FILTER_FORMAT]);
                break;
        }
        ...
        if (\Drupal::moduleHandler()->moduleExists('config_translation') && preg_match('/^entity.webform.(?:edit|settings|assets|access|handlers|third_party_settings)_form$/', \Drupal::routeMatch()->getRouteName() ?? '')) {
            ...
            if ($webform->getLangcode() !== $language_manager->getCurrentLanguage()->getId()) {
                ...
                if ($original_language) {
                    $form['langcode_message'] = ['#type' => 'webform_message', '#message_type' => 'warning', '#message_message' => t('You are editing the original %language language for this webform.', ['%language' => $original_language->getName()]), '#message_close' => TRUE, '#message_storage' => \Drupal\webform\Element\WebformMessage::STORAGE_LOCAL, '#message_id' => $webform->id() . '.original_language', '#weight' => -100];
                }
            }
        }
