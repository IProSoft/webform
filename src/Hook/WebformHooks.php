...
class WebformHooks
{
    ...
    public function configSchemaInfoAlter(&$definitions)
    {
        ...
        if (isset($definitions['webform.variant.override'])) {
            $definitions['webform.variant.override']['mapping'] += ['settings' => $mapping['settings'], 'elements' => $mapping['elements'], 'handlers' => $mapping['handlers']];
        }
    ...
    public function menuLocalTasksAlter(&$data, $route_name, \Drupal\Core\Cache\RefinableCacheableDependencyInterface $cacheability)
    {
        ...
        // test a webform.
        $route_names = ['entity.webform.test_form' => 'entity.webform.canonical', 'entity.node.webform.test_form' => 'entity.node.canonical'];
    ...
    public function mail($key, &$message, $params)
    {
        ...
        $params += ['from_mail' => '', 'from_name' => '', 'cc_mail' => '', 'bcc_mail' => '', 'reply_to' => '', 'return_path' => '', 'sender_mail' => '', 'sender_name' => ''];
    ...
    public function pageAttachments(array &$attachments)
    {
        ...
        if (!\Drupal::moduleHandler()->moduleExists('metatag') && !empty(\Drupal::config('webform.settings')->get('settings.default_confirmation_noindex')) && preg_match('/^(entity\.webform\.confirmation|entity\.[a-z-_]+\.webform\.confirmation)$/', $route_name)) {
            $attachments['#attached']['html_head'][] = [['#tag' => 'meta', '#attributes' => ['name' => 'robots', 'content' => 'noindex']], 'webform_confirmation_noindex'];
        }
    }
    /**
    ...
    {
        $links = ['entitywebformtest-form', 'entitywebformresults-submissions', 'entitywebformedit-form', 'entitywebformsettings'];
    ...
    {
        return ['create' => ['title' => t('Create submissions'), 'roles' => ['anonymous', 'authenticated']], 'view_any' => ['title' => t('View any submissions')], 'update_any' => ['title' => t('Update any submissions')], 'delete_any' => ['title' => t('Delete any submissions')], 'purge_any' => ['title' => t('Purge any submissions')], 'view_own' => ['title' => t('View own submissions')], 'update_own' => ['title' => t('Update own submissions')], 'delete_own' => ['title' => t('Delete own submissions')], 'administer' => ['title' => t('Administer webform & submissions'), 'description' => ['#type' => 'webform_message', '#message_type' => 'warning', '#message_message' => t('<strong>Warning</strong>: The below settings give users, permissions, and roles full access to this webform and its submissions.')]], 'test' => ['title' => t('Test webform')], 'configuration' => ['title' => t('Access webform configuration'), 'description' => ['#type' => 'webform_message', '#message_type' => 'warning', '#message_message' => t("<strong>Warning</strong>: The below settings give users, permissions, and roles full access to this webform's configuration via API requests.")]]];
    }
    /* ************************************************************************** */
    // Element info hooks.
    /* ************************************************************************** */
    ...
    public function elementInfoAlter(array &$info)
    {
        ...
        if (isset($info['text_format'])) {
            $info['text_format']['#process'] = array_merge($info['text_format']['#process'] ?? [], [['\Drupal\webform\Element\WebformHtmlEditor', 'processTextFormat']]);
            $info['text_format']['#pre_render'] = array_merge([['\Drupal\webform\Element\WebformHtmlEditor', 'preRenderTextFormat']], $info['text_format']['#pre_render'] ?? []);
        }
        if (isset($info['processed_text'])) {
            $info['processed_text']['#pre_render'][] = ['\Drupal\webform\Element\WebformHtmlEditor', 'preRenderProcessedText'];
        }
    }
}
