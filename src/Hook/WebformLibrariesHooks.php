...
class WebformLibrariesHooks
{
    ...
    public function libraryInfoBuild()
    {
        ...
        foreach ($webforms as $webform_id => $webform) {
            ...
            foreach ($assets as $type => $value) {
                ...
                if ($type === 'css') {
                    $libraries["webform.css.{$webform_id}"] = ['css' => ['theme' => ["{$base_path}webform/css/{$webform_id}/custom.css?{$default_query_string}" => $settings]]];
                } else {
                    $libraries["webform.javascript.{$webform_id}"] = ['js' => ["{$base_path}webform/javascript/{$webform_id}/custom.js?{$default_query_string}" => $settings]];
                }
            }
        }
        return $libraries;
     ...
     */
    #[Hook('css_alter')]
    public function cssAlter(&$css, \Drupal\Core\Asset\AttachedAssetsInterface $assets)
    {
        // Remove the off-canvas CSS reset for webform admin routes.
        // @see https://www.drupal.org/project/drupal/issues/2826722
        if (\Drupal\webform\Utility\WebformDialogHelper::useOffCanvas()) {
