...
class WebformMaillogHooks
{
    ...
    public function maillogWebformSubmissionFormAlter(array &$form, \Drupal\Core\Form\FormStateInterface $form_state, $form_id)
    {
        ...
        if (\Drupal::currentUser()->hasPermission('administer maillog')) {
            $t_args = [':href' => \Drupal\Core\Url::fromRoute('maillog.settings')->toString()];
            if ($config->get('log')) {
                $build[] = [
