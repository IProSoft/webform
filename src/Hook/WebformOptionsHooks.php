...
class WebformOptionsHooks
{
    ...
    public function webformOptionsRangeAlter(array &$options, array $element = [])
    {
        ...
        $options = \Drupal\webform\Utility\WebformOptionsHelper::range($element['#min'], $element['#max'], $element['#step'], $element['#pad_length'], $element['#pad_str']);
    }
    /**
    ...
    {
        if (empty($options)) {
            $languages = \Drupal\Core\Language\LanguageManager::getStandardLanguageList();
