...
class WebformQueryHooks
{
    ...
    public function queryWebformSubmissionListBuilderAlter(\Drupal\Core\Database\Query\AlterableInterface $query)
    {
    ...
    public function queryEntityReferenceAlter(\Drupal\Core\Database\Query\AlterableInterface $query)
    {
    ...
    public function queryWebformSubmissionAccessAlter(\Drupal\Core\Database\Query\AlterableInterface $query)
    {
        /** @var \Drupal\Core\Database\Query\SelectInterface $query */
        ...
        if ($account->isAuthenticated()) {
            ...
            if ($cached) {
                ...
                \Drupal::cache()->set('webform_submission_access__account_update__' . $account->id(), $webform_account_access_update, \Drupal\Core\Cache\Cache::PERMANENT, ['config:webform_list', 'user:' . $account->id()]);
            }
        ...
        if ($cached) {
            ...
            \Drupal::cache()->set('webform_submission_access__webform_access_rules', $webform_access_rules, \Drupal\Core\Cache\Cache::PERMANENT, ['config:webform_list']);
        }
