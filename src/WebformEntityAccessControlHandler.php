  ...
  public function checkAccess(EntityInterface $entity, $operation, AccountInterface $account) {
    ...
    if ($account->isAuthenticated()) {
      switch ($operation) {
        ...
        case 'duplicate':
          if ($account->hasPermission('create webform') && ($entity->isTemplate() || ($account->hasPermission('edit any webform') || ($account->hasPermission('edit own webform') && $is_owner))) || ($account->hasPermission('duplicate any webform'))) {
            return WebformAccessResult::allowed($entity, TRUE);
          }
          break;
