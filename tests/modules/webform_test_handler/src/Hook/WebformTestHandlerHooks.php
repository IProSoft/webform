...
class WebformTestHandlerHooks
{
    ...
    {
        return ['webform_handler_test_summary' => ['variables' => ['settings' => NULL, 'handler' => []]], 'webform_handler_test_offcanvas_width' => ['variables' => ['settings' => NULL, 'handler' => []]]];
    }
    /**
    ...
    public function webformSubmissionsPrePurge(array &$webform_submissions)
    {
        ...
        \Drupal::state()->set('webform_test_purge_hook_pre', array_map(function (\Drupal\webform\WebformSubmissionInterface $submission) {
    ...
    {
        \Drupal::state()->set('webform_test_purge_hook_post', array_map(function (\Drupal\webform\WebformSubmissionInterface $submission) {
            return $submission->id();
        }, $webform_submissions));
    }
}
