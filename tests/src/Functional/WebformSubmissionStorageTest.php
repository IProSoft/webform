...
class WebformSubmissionStorageTest extends WebformBrowserTestBase {
  ...
  public function testSubmissionStorage() {
    ...
    // Check total.
    $this->assertEquals($storage->getTotal($webform), 6);
    $this->assertEquals($storage->getTotal($webform, NULL, $user1), 3);
    $this->assertEquals($storage->getTotal($webform, NULL, $user2), 3);

    // Check next submission.
    $this->drupalLogin($user1);
    $this->assertEquals($storage->getNextSubmission($user1_submissions[0], NULL, $user1)->id(), $user1_submissions[1]->id(), "User 1 can navigate forward to user 1's next submission");
    ...
    $this->assertEquals($storage->getNextSubmission($user1_submissions[2], NULL)->id(), $user2_submissions[0]->id(), "Admin user can navigate between user submissions");
    $this->drupalLogout();

    // Check previous submission.
    $this->drupalLogin($user1);
    $this->assertEquals($storage->getPreviousSubmission($user1_submissions[1], NULL, $user1)->id(), $user1_submissions[0]->id(), "User 1 can navigate backward to user 1's previous submission");
    $this->drupalLogin($user2);
    ...
    $this->drupalLogin($admin_user);
    $this->assertEquals($storage->getPreviousSubmission($user2_submissions[0], NULL)->id(), $user1_submissions[2]->id(), "Admin user can navigate between user submissions");
    $this->drupalLogout();
    ...
    $this->assertEquals($storage->getTotal($webform), 8);
    $this->assertEquals($storage->getTotal($webform, NULL, $user1), 4);
    $this->assertEquals($storage->getTotal($webform, NULL, $user2), 4);
  }
