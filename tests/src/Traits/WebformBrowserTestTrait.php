...
trait WebformBrowserTestTrait {
  ...
  protected function placeWebformBlocks($module_name) {
    $config_directory = \Drupal::service('extension.list.module')->getPath('webform') . '/tests/modules/' . $module_name . '/config';
  ...
  protected function loadWebform($id) {
    ...
    if ($webform = $storage->load($id)) {
      ...
      if (strpos($id, 'test_') === 0) {
        $config_directory = \Drupal::service('extension.list.module')->getPath('webform') . '/tests/modules/webform_test/config/install';
      }
      elseif (strpos($id, 'example_') === 0) {
        $config_directory = \Drupal::service('extension.list.module')->getPath('webform') . '/modules/webform_examples/config/install';
      }
      elseif (strpos($id, 'template_') === 0) {
        $config_directory = \Drupal::service('extension.list.module')->getPath('webform') . '/modules/webform_templates/config/install';
      }
      else {
